...
import GlobalPlatformClient from '../GlobalPlatformClient';
...
describe('GlobalPlatformClient', () => {
    const baseOptions: GlobalPlatformClientOptions = {
    ...
    test('2 API objects are created when creating a platform instance', () => {
        new GlobalPlatformClient(baseOptions);
        expect(APIMock).toHaveBeenCalledTimes(2);
    });

    test('the API uses an empty organization id', () => {
        new GlobalPlatformClient(baseOptions);
    ...
    it('should abort get requests on the API instance', () => {
        const platform = new GlobalPlatformClient(baseOptions);
    ...
    it('should register all the resources on the platform instance', () => {
        const registerAllSpy = jest.spyOn(GlobalPlatformResources.prototype, 'registerAll');
        new GlobalPlatformClient(baseOptions);
    ...
    describe('withFeatures', () => {
        it('should create a copy of the client with the new feature', async () => {
            ...
            const client = new GlobalPlatformClient(baseOptions);

            const clientWithFeature = client.withFeatures(feature);

            expect(clientWithFeature).toBeInstanceOf(GlobalPlatformClient);
